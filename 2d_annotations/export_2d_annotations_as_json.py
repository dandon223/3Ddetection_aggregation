...
# copied from https://github.com/nutonomy/nuscenes-devkit/blob/master/python-sdk/nuscenes/scripts/export_2d_annotations_as_json.py
...
def get_nusc_scenes(nusc: NuScenes) -> List:
    """Retruns list of scenes with sample tokens
...
def main(args):
    ...
    for reprojection in reprojections:
        sd_rec = nusc.get('sample_data', reprojection['sample_data_token'])
        s_rec = nusc.get('sample', sd_rec['sample_token'])
        if 'car' in reprojection['category_name']:
            if s_rec['token'] in sample_tokens:
                sample_tokens[s_rec['token']].append(reprojection)
            else:
                sample_tokens[s_rec['token']] = [reprojection]
    ...
    reprojections = {'dataroot': args.dataroot,
                     'version': args.version, 'sequences': sequences}
    print("Saving the 2D re-projections under {}".format(args.output_file))
    # Save to a .json file.
    ...
    print("Saved the 2D re-projections under {}".format(args.output_file))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version', type=str,
                        default='v1.0-trainval', help='Dataset version.')
    parser.add_argument('--output_file', type=str,
                        default='./image_annotations.json', help='Output filename.')
